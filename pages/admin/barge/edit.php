...
if (!isset($_SESSION['nama'])) {
    ...
    $id = $_GET['id'];
    $query = $link->query("SELECT b.*, jk.namaJenisKapal, jm.jenisMesin, f.namaFlag 
    FROM barge b
    JOIN jenis_kapal jk ON b.idJenisKapal = jk.idJenisKapal
    JOIN jenis_mesin jm ON b.idJenisMesin = jm.idJenisMesin
    JOIN flag f ON b.idFlag = f.idFlag WHERE idBarge = '$id'");
...
?>
    ...
    if (isset($_POST['edit'])) {
        ...
        $edit = $link->query("UPDATE barge SET 
...
WHERE idBarge = '$id'");
    ...
    Swal.fire({
        ...
        window.location.href = '?page=data_barge'; 
    });
</script>";
        } else {
            echo "<script>
    Swal.fire({
        ...
        window.location.href = '?page=edit_barge'; 
    });
</script>";
        }
    }
