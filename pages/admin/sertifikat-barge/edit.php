...
if (!isset($_SESSION['nama'])) {
    ...
    $id = $_GET['id'];
    $query = $link->query("SELECT sb.*, js.namaSertifikat, b.namaKapal   
    FROM sertifikat_barge sb
    JOIN jenis_sertifikat js ON sb.idJenisSertifikat = js.idJenisSertifikat
    JOIN barge b ON sb.idBarge = b.idBarge WHERE idSertifikatBarge = '$id'");
...
?>
                                    ...
                                    <div class="mb-3 col-md-6">
                                        ...
                                        <div class="input-groupp input-group-merge">
                                            ...
                                            <select id="multicol-ships" class="select2 form-select" data-allow-clear="true" name="idBarge" required>
                                                ...
                                                <?php $q = $link->query("SELECT * FROM barge ");
                                                while ($d =
                                                    $q->fetch_array()
                                                ) {
                                                    if ($d['idBarge'] == $data['idBarge']) { ?>
                                                        <option value="<?= $d['idBarge']; ?>" selected="<?= $d['idBarge']; ?>">
                                                            <?= $d['namaKapal'] ?>
                                                        </option>
                                                    <?php
                                                    } else {
                                                    ?>
                                                        <option value="<?= $d['idBarge'] ?>">
                                                            <?= $d['namaKapal'] ?>
                                                        </option>
                                                <?php }
                                                } ?>
    ...
    if (isset($_POST['edit'])) {
        ...
        if ($link) {
            $stmt = $link->prepare("UPDATE sertifikat_barge SET idJenisSertifikat = ?, tglTerbit = ?, tglExp = ?, permanent = ?, berkas = ? WHERE idSertifikatBarge = ?");

            if ($stmt) {
                $stmt->bind_param("issssi", $idJenisSertifikat, $tglTerbit, $tglExp, $permanent, $berkas, $idSertifikatBarge);
    ...
    Swal.fire({
        ...
        window.location.href = '?page=edit_sertBarge'; 
    });
</script>";
        }
    }
