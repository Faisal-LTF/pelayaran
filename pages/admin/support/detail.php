...
if (!isset($_SESSION['nama'])) {
    ...
    $id = $_GET['id'];
    $queryCombined = $link->query("SELECT s.*, jk.namaJenisKapal, jm.jenisMesin, f.namaFlag
FROM support s
JOIN jenis_kapal jk ON s.idJenisKapal = jk.idJenisKapal
JOIN jenis_mesin jm ON s.idJenisMesin = jm.idJenisMesin
JOIN flag f ON s.idFlag = f.idFlag
WHERE s.idSupport = '$id'");
    $data = $queryCombined->fetch_array();
    $certificatesQuery = $link->query("SELECT ss.*, js.namaSertifikat, s.namaKapal
    FROM sertifikat_support ss
    JOIN jenis_sertifikat js ON ss.idJenisSertifikat = js.idJenisSertifikat
    JOIN support s ON ss.idSupport = s.idSupport
    WHERE ss.idSupport = '$id'
");
